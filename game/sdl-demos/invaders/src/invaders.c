...
int main(int argc, char *argv[]){
        ...
        if(init() < 0){
                ...
                tmpFSM.gameStates = &states;
		tmpFSM.pushState = pushState;
		tmpFSM.changeState = changeState;
		tmpFSM.popState = popState;
		tmpFSM.popAllStates = popAllStates;

                fsm = &tmpFSM;

                playState.update = playUpdate;
		playState.render = playDraw;
		playState.onEnter = playOnEnter;
		playState.onExit = playOnExit;
		playState.stateId = GAME_STATE_PLAY;
