...
static void y_printres(yxml_t *x, yxml_ret_t r, List *bricks) {
	static int indata;
	int nextdata = 0;
	...
	case YXML_ELEMSTART:
		break;
	case YXML_ELEMEND:
		addBrick(bricks, pos_x, pos_y, w, h, "graphics/bricks.png");
		break;
	case YXML_ATTREND:
		if(!strcmp("x", x->attr)){
			pos_x = atoi(pos_xChr);
...
int parseBricks(List *bricks, char *filename,
	...
	while((c = getc(fp)) != EOF) {
		r = yxml_parse(x, c);
		y_printres(x, r, bricks);
	}

	fclose(fp);
	return list_size(bricks);
}
